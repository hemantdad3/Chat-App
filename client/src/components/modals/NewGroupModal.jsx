  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/40 backdrop-blur-xs animate-fadeIn">
      <div className="w-full max-w-md bg-sand-light border border-ink-border rounded-2xl p-6 shadow-xl flex flex-col max-h-[90vh]">
        ...
        {/* Group Name Input */}
        <div className="mt-4">
          <label className="block text-xs font-medium text-ink mb-1.5" htmlFor="groupName">
            Group Name
          </label>
          <input
            id="groupName"
            ...
            className="w-full bg-cream border border-ink-border text-ink text-sm rounded-xl px-4 py-2.5 placeholder-ink-faint focus:outline-none focus:border-terracotta focus:ring-1 focus:ring-terracotta transition-colors"
          />
        </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1.5 max-h-20 overflow-y-auto p-1">
            {selectedUserIds.map((userId) => {
              ...
              return (
                <span
                  key={userId}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-sand border border-ink-border text-ink rounded-lg text-xs"
                >
                  <span>{u?.name || 'User'}</span>
                  <button
                    type="button"
                    onClick={() => toggleUserSelection(userId)}
                    className="hover:text-rose-600 text-ink-muted cursor-pointer"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </span>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 min-h-[180px]">
          {loadingUsers ? (
            ...
            <div className="flex flex-col items-center justify-center h-32 text-ink-muted text-xs">
              No contacts found
            </div>
          ) : (
        ...
        {/* Footer Actions */}
        <div className="mt-4 pt-4 border-t border-ink-border flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            className="px-4 py-2 bg-terracotta hover:bg-terracotta-hover disabled:bg-sand disabled:text-ink-faint text-white text-xs font-medium rounded-xl flex items-center gap-2 transition-all shadow-sm cursor-pointer disabled:cursor-not-allowed"
          >
